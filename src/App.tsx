      ...
      return (
        <div className="space-y-4">
          ...
          <div className="bg-amber-50 dark:bg-amber-900/20 p-4 rounded-lg border border-amber-200 dark:border-amber-800 text-amber-800 dark:text-amber-200 text-sm">
            Ga naar het <strong>Beheren &gt; Accounts</strong> scherm (als applicatiebeheerder) of stuur een e-mail naar de applicatiebeheerder om dit account te bewerken, de profielfoto te verwijderen, en het e-mailadres te wissen.
          </div>
          {userToAnonymize && (
    ...
    const unsub = onSnapshot(doc(db, "appData", "main"), (docSnap) => {
      if (docSnap.exists()) {
        try {
          ...
          const migratedDataStr = JSON.stringify(parsed);
                  ...
                  <div className="space-y-6">
                    {data.seasons
                      ...
                      .map((season: Season) => {
                        ...
                        const isCollapsed = collapsedCashbookSeasons.includes(season.id);
                        return (
                          <div
                            key={season.id}
...
{[...matches].sort((a: Match, b: Match) => {
            ...
            {activeTab === "manage-accounts" && currentUser.role === "applicatiebeheerder" && (
...
{isInstallable && (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="p-6 bg-slate-50 dark:bg-slate-800/50 flex gap-3">
                ...
                <button
                  disabled={!newMemberName || !newMemberEmail || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(newMemberEmail)}
                  onClick={() => {
                    if (editingMemberId) {
                      updateMember(
                        editingMemberId,
      ...
      <AnimatePresence>
        ...
        {isContributionsDetailModalOpen && (data.seasons.find((s: Season) => s.id === contributionsDetailSeasonId) || activeSeason) && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="p-6 max-h-[70vh] overflow-y-auto">
                {contributionsDetailType === "contributions" && (
                  <div className="space-y-3">
                    {((data.seasons.find((s: Season) => s.id === contributionsDetailSeasonId) || activeSeason).members || []).map((m) => {
                      const user = data.users.find(
                        (u: User) => u.id === m.userId,
                      );
                      return (
                        ...
                        >
                          ...
                          <div className="text-right">
                            ...
                            >
                              {m.paidContributie
                                ? formatCurrency((data.seasons.find((s: Season) => s.id === contributionsDetailSeasonId) || activeSeason).contributie)
                                : formatCurrency(0)}
                            </p>
                          </div>
                        </div>
                    ...
                    <div className="space-y-3">
                      {matchfeesGroupView === "player"
                        ? ((data.seasons.find((s: Season) => s.id === contributionsDetailSeasonId) || activeSeason).members || []).map((m) => {
                            ...
                            const memberMatches = data.matches.filter(
                              (match: Match) =>
                                match.seasonId === (data.seasons.find((s: Season) => s.id === contributionsDetailSeasonId) || activeSeason).id &&
                                match.status !== "cancelled" &&
                            ...
                            const totalDue =
                              playedMatches * (data.seasons.find((s: Season) => s.id === contributionsDetailSeasonId) || activeSeason).inlegPerWedstrijd;
                            const totalPaid =
                              paidMatches * (data.seasons.find((s: Season) => s.id === contributionsDetailSeasonId) || activeSeason).inlegPerWedstrijd;
                            const open = totalDue - totalPaid;
                        ...
                        : (() => {
                            ...
                            return Object.entries(
                              ...
                              .map(([dateStr, matches]) => {
                                ...
                                const totalDue =
                                  playedMatchesCount *
                                  (data.seasons.find((s: Season) => s.id === contributionsDetailSeasonId) || activeSeason).inlegPerWedstrijd;
                                let totalPaid = 0;
                                matches.forEach((m: Match) => {
                                  if (m.player1Paid)
                                    totalPaid += (data.seasons.find((s: Season) => s.id === contributionsDetailSeasonId) || activeSeason).inlegPerWedstrijd;
                                  if (m.player2Paid)
                                    totalPaid += (data.seasons.find((s: Season) => s.id === contributionsDetailSeasonId) || activeSeason).inlegPerWedstrijd;
                                });
                                const open = totalDue - totalPaid;
                ...
                {contributionsDetailType === "externalmatchfees" && (
                  <div className="space-y-3">
                    {data.externalMatches
                      ?.filter((em: any) => em.seasonId === (data.seasons.find((s: Season) => s.id === contributionsDetailSeasonId) || activeSeason).id)
                      .map((em: any) => {
                        let totalDue = 0;
                        let totalPaid = 0;
