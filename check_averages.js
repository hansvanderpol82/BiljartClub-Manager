...
async function run() {
    ...
    matches.forEach(m => {
        if (m.player1Id === antoon.id) avgSet.add(m.player1AvgBefore);
        if (m.player2Id === antoon.id) avgSet.add(m.player2AvgBefore);
    });
    
    console.log("Distinct averages for 'Antoon':", Array.from(avgSet));
    process.exit(0);
}
run();
