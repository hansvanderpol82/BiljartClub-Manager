...
// 1. Update handleGenerateTemplate
...
// 2. Update date reading in handleFileUpload
const oldDateRead = `date: w["Datum (YYYY-MM-DD)"] || new Date().toISOString().split('T')[0],`;

const newDateRead = `date: (() => {
...
content = content.replace(oldDateRead, newDateRead);

fs.writeFileSync('src/components/ExcelModule.tsx', content);
console.log("Updated Excel module successfully!");
